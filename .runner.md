...
Create local config file once:

```bash
cp service/src/main/resources/serverConfig.yml service/src/main/resources/localconfig.yaml
```

Notes:
- Local runtime prefers `service/src/main/resources/localconfig.yaml` when present.
- Production runtime uses `serverConfig.yml` (or explicit `SERVER_CONFIG_PATH`/`SERVER_CONFIG_FILE`).
